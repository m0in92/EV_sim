...
class SubInputFrame(ttk.Frame):
    ...
    def create_bindings(self):
        def lstbox_callback(event, category):
        ...
        for category in self.lstbox_choices.keys():
            exec('self.lstbox_' + category.replace(' ', '_') + f".bind('<<ListboxSelect>>', lambda e:lstbox_callback(e,'{category}'))",
                 locals())
...
class MainDisplayFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        # Widgets
        self.create_widget(main_heading_text="Default")

        # Layout
        self.grid(row=1, column=1, sticky='news')

    def create_widget(self, main_heading_text):
...
#         self.grid(row=start_row_num, column=0, sticky=(tkinter.N,tkinter.W), pady=(10, 0))
#
